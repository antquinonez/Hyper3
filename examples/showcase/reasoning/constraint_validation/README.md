...
graph LR
    ...
    subgraph "Constraint Pipeline"
        C1["NoSelfLoop"] --> C2["WeightInflation"] --> C3["ProvenanceDepth"] --> C4["DuplicateEdge"]
    end

    subgraph "Partition"
        C4 --> A["Accepted (11)"]
        C4 --> R["Rejected (4)<br/>+ violation details"]
    end
```
...
- `DuplicateEdgeConstraint` compares the proposed edge's source_ids, target_ids, and label against every existing edge. An exact match on all three triggers rejection. The check uses edge identity (`existing.id != edge.id`) to avoid flagging an edge as a duplicate of itself.
...
**Section 5 — Batch validation:** 15 edge proposals are submitted to `validate_and_filter()`, which returns two lists: 11 valid and 4 rejected. The rejection breakdown shows 2 self-loops, 1 weight inflation, and 1 provenance depth violation. No `DuplicateEdgeConstraint` rejections appeared because the only proposal matching an existing source/target/label triple is `existing_edge` — the actual graph edge object reused from Section 3 as a test fixture. Since `DuplicateEdgeConstraint` compares edge IDs (`existing.id != edge.id`), an edge cannot be a duplicate of itself. A genuinely new edge with the same source, target, and label would be caught. Why this matters: batch validation is the production workflow. Rather than checking edges one at a time, the pipeline partitions the entire proposal list into accepted and rejected with per-edge violation details, enabling "accept the good, log the bad" processing without stopping on the first failure.
