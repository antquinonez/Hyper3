...
```bash
.venv/bin/python examples/showcase/belief_and_bayesian/25_belief_and_bayesian.py
```
...
|---------|-----------|-----------|--------------|----------------------|
| `river_edge` | 0.7 | 0.49 | 0.521 | 53.0% |
| `financial` | 0.6 | 0.36 | 0.383 | 34.0% |
| `billiards` | 0.3 | 0.09 | 0.096 | 13.0% |
...
Two distributions are created and linked with pairwise correlation coefficients. The "bank" distribution (`financial`, `river_edge`, `billiards`) is correlated with a water-context distribution (`drinking`, `river`, `ocean`). High positive correlation between `river_edge` and `river` (0.9) means that sampling `river_edge` from the bank distribution biases the water distribution toward `river`, and vice versa.

Sampling from the bank distribution yields `river_edge`. If the water distribution were then sampled, the correlation structure would make `river` more probable than its base amplitude suggests.
...
After calling `reset_belief`, the distribution returns to uniform (0.333 each). This allows reusing the same concept for a new inference session without creating a new node.
