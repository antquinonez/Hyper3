...
**Rule-based reasoning** infers new edges from structural patterns. The `TransitiveRule` on `collaborates` detects two-hop chains: alice -[collaborates]-> bob -[collaborates]-> carol produces an `indirect_collaboration` edge from alice to carol. The multiway expansion engine explores all possible rule applications simultaneously, producing 2 states and 1 new edge. This matters because alice and carol have no direct `collaborates` edge — the transitive rule makes their indirect relationship queryable alongside direct ones. Without reasoning, finding alice's "collaborators of collaborators" would require manual graph traversal.
