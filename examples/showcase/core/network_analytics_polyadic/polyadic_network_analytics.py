...
A variant of graph_analytics.py that models naturally collective relationships
as n-ary (polyadic) hyperedges instead of pairwise links. Where the original
creates N separate ``link(host, svc, label="runs")`` edges per host, this version
creates one ``link_hyper(sources={host}, targets={svc1, svc2, ...})`` edge per
host -- a single hyperedge capturing "host runs this *set* of services."

The same 129-node enterprise topology is used, but edges for services,
exposures, vulnerabilities, protections, trust groups, and access are polyadic.
Host-to-segment and segment-to-segment routing remain pairwise because they are
naturally 1:1 or directed point-to-point relationships.

An additional Section 13 demonstrates polyadic-specific analysis:
``query_hyperedges``, ``hyperedge_neighbors``, ``unique_edge_sizes``, and
``max_edge_order``.
...
def build_edges(mem: HypergraphMemory) -> int:
    ...
    for host, svcs in host_services.items():
        mem.link_hyper(sources={host}, targets=set(svcs), label="runs", weight=1.0)
        edges += 1
    ...
    for host, svcs in exposure.items():
        mem.link_hyper(sources={host}, targets=set(svcs), label="exposed_on", weight=1.0)
        edges += 1
    ...
    for host, vulns in vuln_map.items():
        mem.link_hyper(sources={host}, targets=set(vulns), label="vulnerable_to", weight=1.0)
        edges += 1
    ...
    for host, ctrls in protections.items():
        mem.link_hyper(sources={host}, targets=set(ctrls), label="protected_by", weight=1.0)
        edges += 1
    ...
    mem.link_hyper(
        sources={"dc-01", "dc-02"}, targets={"admin-bastion"},
        label="trust_group", weight=1.0,
    )
    edges += 1
    mem.link_hyper(
        sources={"admin-bastion"}, targets={"dc-01", "dc-02"},
        label="trust_group", weight=1.0,
    )
    edges += 1
    mem.link_hyper(
        sources={"app-01", "app-02"}, targets={"db-primary", "db-replica"},
        label="trust_group", weight=1.0,
    )
    edges += 1

    access_by_user: dict[str, list[str]] = defaultdict(list)
    access = [
    ...
    for user, host in access:
        access_by_user[user].append(host)
    for user, hosts in access_by_user.items():
        mem.link_hyper(sources={user}, targets=set(hosts), label="has_access", weight=1.0)
        edges += 1
    ...
    for edge in vuln_edges:
        for lbl in edge.source_labels:
            if lbl in host_labels:
                vuln_count[lbl] += len(edge.target_labels)
...
def find_cross_zone_violations(mem: HypergraphMemory) -> list[tuple[str, str, str, str, str]]:
    ...
    for edge in route_edges:
        for src_label in edge.source_labels:
            for tgt_label in edge.target_labels:
...
def main():
    ...
    # =====================================================================
    # SECTION 1: Building Enterprise Network Topology (Polyadic Edges)
    # =====================================================================
    print("=" * 70)
    print("SECTION 1: Building Enterprise Network Topology (Polyadic Edges)")
    print("=" * 70)
    ...
    print()
    print("  Note: N-ary hyperedges contribute to degree for ALL participant")
    print("  nodes. A polyadic 'runs' edge {host} -> {svc1, svc2, svc3} gives")
    print("  each of the four nodes one incident edge, not three separate edges.")
    print()
    ...
    if len(components) == 1:
        ...
        print("  N-ary edges unify all participant nodes into shared components.")
    else:
    ...
    dmz_with_vulns = [(h, risk_scores.get(h, 0)) for h in dmz_hosts if vuln_count.get(h, 0) > 0]
    ...
    # =====================================================================
    print("=" * 70)
    print("SECTION 13: Polyadic Analysis")
    print("=" * 70)

    n_ary_edges = mem.query_hyperedges(min_source_cardinality=2)
    print(f"  True n-ary edges (source cardinality >= 2): {len(n_ary_edges)}\n")

    for edge in n_ary_edges:
        src_labels = ", ".join(
            n.label for sid in edge.source_ids if (n := mem.engine.graph.get_node(sid))
        )
        tgt_labels = ", ".join(
            n.label for tid in edge.target_ids if (n := mem.engine.graph.get_node(tid))
        )
        print(f"    ({{{src_labels}}}) -> {{{tgt_labels}}}  label=\"{edge.label}\"")
    print()

    print("  Hyperedge neighbors of dc-01 (co-occurrence in n-ary edges):")
    neighbors = mem.hyperedge_neighbors("dc-01")
    for nbr_label, shared_edges in sorted(neighbors.items()):
        edge_labels = [e.label for e in shared_edges]
        unique_labels = sorted(set(edge_labels))
        print(f"    {nbr_label:20s}: {len(shared_edges)} shared edges ({', '.join(unique_labels)})")
    print()

    edge_sizes = mem.engine.graph.unique_edge_sizes()
    max_order = mem.engine.graph.max_edge_order()
    print(f"  Unique edge sizes (source+target cardinality): {sorted(edge_sizes)}")
    print(f"  Maximum edge order: {max_order}")

    total_edges = mem.size[1]
    pairwise_count = sum(
        1 for e in mem.engine.graph.edges
        if len(e.source_ids) == 1 and len(e.target_ids) == 1
    )
    polyadic_count = total_edges - pairwise_count
    print("\n  Edge breakdown:")
    print(f"    Pairwise edges: {pairwise_count}")
    print(f"    N-ary edges:    {polyadic_count}")
    ...
    print("SUMMARY")
    print("=" * 70)
    print(f"  Network: {mem.size[0]} nodes, {mem.size[1]} edges ({polyadic_count} n-ary)")
