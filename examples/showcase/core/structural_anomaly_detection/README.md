...
```bash
.venv/bin/python examples/showcase/core/structural_anomaly_detection/structural_anomaly_detection.py
```
...
auth_svc:
  ...
  warnings: ['Cyclic structure detected - analysis may be incomplete', 'Contradictory edge labels detected - results may be inconsistent']
...
suggested assumptions for 'config_svc': 3
  bridge_69f0495e: Assume reachability to api_gateway (coverage gain: 0.5000)
  bridge_bcabe422: Assume reachability to order_svc (coverage gain: 0.3889)
  bridge_3b6aae70: Assume reachability to db_primary (coverage gain: 0.2222)
...
The showcase produces three levels of anomaly analysis, each answering a different question:

1. **Per-node assessment** (Section 2) — Calls `detect_structural_anomalies()` on individual nodes. Returns the boundary score, anomaly status, and specific warnings. Use this when investigating a specific service. Example: auth_svc has boundary score 0.3975 with warnings about cycles and contradictory labels.

2. **Boundary region mapping** (Section 4) — Calls `map_boundaries()` on the entire graph. Returns a classification count (14 low_risk, 0 boundary, 4 anomalous) and lists the anomalous nodes. Use this for an at-a-glance health overview of the whole system.

3. **Cross-reference with centrality** (Section 6) — Computes betweenness centrality for all nodes and pairs each with its anomaly status. Reveals which high-centrality nodes are healthy (api_gateway, a designed hub) versus structurally risky (order_svc, a cycle participant on many paths). This distinction is invisible to centrality analysis alone.
...
**Section 2 — Individual anomaly assessment:** The script calls `detect_structural_anomalies()` on six representative services. api_gateway is classified as low_risk (boundary score 0.0794) despite having the highest centrality — it has no cycles, no contradictions, and no structural anomalies. The anomaly detector considers centrality as one of four indicators, weighted at 0.3, so high centrality alone (0.2647 × 0.3 = 0.0794) does not cross the anomaly threshold of 0.3. auth_svc is classified as anomalous (boundary score 0.3975) with warnings about cyclic structure and contradictory edge labels. user_svc and order_svc are also anomalous with similar scores, confirming the planted circular dependency was detected. logging_svc is low_risk (0.0265) — its multi-database connections do not trigger any anomaly indicators. config_svc is low_risk (0.0088), the lowest score in the graph. Why this matters: the detector correctly distinguishes the cycle participants (anomalous) from the high-throughput gateway (low risk) and the well-connected logger (low risk). Not every well-connected node is anomalous — only those with genuine structural problems.

**Section 3 — Boundary indicator deep dive:** The script inspects the four individual indicator dimensions for api_gateway, auth_svc, logging_svc, and config_svc.

- **api_gateway** — high_centrality=0.2647 (highest in the graph) but all other indicators are 0.0. It is a hub, not a structural problem.
- **auth_svc** — cyclic_structure=0.8000 (participates in a strongly connected component), high_centrality=0.1250, and contradiction_risk=0.6000. The contradiction risk comes from the learned opposition heuristic: auth_svc's incident edges carry labels `routes_to` (from infrastructure nodes) and `calls` (from service nodes), and these two labels have completely disjoint source node sets in the graph. When two labels participate in the same node but originate from entirely separate populations, the detector flags this as a potential semantic inconsistency — `routes_to` implies external routing while `calls` implies internal service-to-service communication, and a node that receives both may have contradictory role expectations.
- **logging_svc** — high_centrality=0.0882 and all others 0.0. Its connections to all three databases do not create contradictions (all edges are `writes_to`, a single label) or cycles.
- **config_svc** — high_centrality=0.0294 and all others 0.0. The lowest boundary score in the graph.

The boundary score formula is `0.3 × cyclic + 0.3 × centrality + 0.2 × contradiction + 0.2 × structural`, giving auth_svc `0.3×0.8 + 0.3×0.125 + 0.2×0.6 + 0.2×0.0 = 0.3975`.

**Why this matters:** The four indicators decompose the anomaly into independent dimensions. A team investigating auth_svc can see that cyclic_structure (0.8) is the dominant contributor and focus on breaking the cycle rather than addressing centrality or contradictions. The contradiction flag adds a secondary signal: auth_svc sits at the junction of two structurally distinct edge populations (infrastructure routing vs. service calls), which amplifies the risk of the circular dependency because failures can propagate through both channels.

**Section 4 — Boundary region mapping:** `map_boundaries()` classifies all 18 nodes: 14 low_risk, 0 boundary, 4 anomalous. The 4 anomalous services are auth_svc (0.3975), user_svc (0.3975), order_svc (0.4218), and db_primary (0.3450).

db_primary's anomaly stems from its structural position: it has the highest centrality of any database (high_centrality=0.75) because it receives both `reads_from` and `writes_to` from cycle participants (user_svc, order_svc) and non-cycle services (logging_svc). The structural_anomaly_score (0.6) reflects this unusual connectivity pattern — a single database serving as both read and write target for multiple services in the cycle. Unlike the cycle participants, db_primary has contradiction_risk=0.0 because `reads_from` and `writes_to` share overlapping source nodes (order_svc does both), so the learned opposition heuristic does not flag them as contradictory.

No nodes fall in the boundary category — the graph topology produces a clear split between healthy and problematic nodes. **Why this matters:** the boundary map provides an at-a-glance health overview of the entire service mesh. A platform team can immediately see which services need attention without scanning individual scores.
