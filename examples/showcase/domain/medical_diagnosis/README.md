...
| High confidence nodes (<0.3) | 0 |
| Nodes below 0.5 | 0 |
| pneumonia confidence | 0.722 (inferred) |
| pulmonary_embolism confidence | ~0.85 (inferred) |
| lung_cancer confidence | 0.722 (inferred) |

Disease-specific confidences vary based on each node's graph topology — the number and structure of `causes` edges, inferred edges, and their weight distribution. Pulmonary embolism has higher confidence than pneumonia or lung cancer because its edge topology produces a stronger evidence base after reasoning.
...
**Sampling results (15 draws):** Sampling is probabilistic (Born-rule); exact frequencies vary between runs. In a typical run, pneumonia dominates with ~11-14 of 15 samples, reflecting its much higher amplitude. The sampling frequency approximates the Born-rule probabilities — pneumonia's theoretical probability of 0.609 makes it the most likely diagnosis on each draw, but small sample sizes produce significant variation around the expected frequency.
...
|---------|--------|---------------|-------------|
| pneumonia | anomalous | ~0.42 | Cyclic dependency structure — many diseases and findings form feedback loops |
| dyspnea | anomalous | ~0.42 | High-scoring symptom — caused by many diseases |
| tachycardia | anomalous | ~0.41 | Non-specific finding — multiple causes and downstream effects |
| cough | anomalous | ~0.41 | Caused by many diseases, diamond convergence point |
| fever | anomalous | ~0.40 | Low-specificity symptom, caused by several diseases |
| pulmonary_embolism | anomalous | ~0.39 | High-connectivity disease, many causes and manifestations |
...
| Sampling draws | 15 |
| Pneumonia samples | ~11-14/15 (probabilistic) |
| Pulmonary embolism samples | ~1-3/15 (probabilistic) |
| Anomaly concepts tested | 6 |
| Anomalous concepts | 6 |
| Highest boundary score | pneumonia (~0.42) |
| Lowest boundary score | pulmonary_embolism (~0.39) |
