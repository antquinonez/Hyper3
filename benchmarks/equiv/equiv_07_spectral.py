...
def _test_laplacian_eigenvalues(t: EquivRunner) -> None:
    ...
    L = np.asarray(mem.engine.graph.hypergraph_laplacian())
    ...
    L_norm, _ = mem.engine.graph.normalized_laplacian()
    ...
    G = build_pairwise_nx()
    L_h3 = np.asarray(mem_pw.engine.graph.hypergraph_laplacian())
    ...
    L_norm_pw, _ = mem_pw.engine.graph.normalized_laplacian()
...
def _test_spectral_embedding(t: EquivRunner) -> None:
    ...
    t.check_int("spectral_embedding/node_count", len(embeddings), mem.engine.graph.node_count)

    for label, vec in embeddings.items():
        t.check(f"spectral_embedding/vec_len/{label}", len(vec) == 3)

    kernel_result = mem.engine.graph.spectral_embedding(dimensions=3)
...
def _test_spectral_clustering(t: EquivRunner) -> None:
    ...
    for cluster in clusters:
        all_nodes |= cluster
    t.check_int("spectral_clustering/covers_all_nodes", len(all_nodes), mem.engine.graph.node_count)
...
def _test_algebraic_connectivity(t: EquivRunner) -> None:
    ...
    h3_ac = mem.engine.graph.algebraic_connectivity()
    ...
    hg_ac = mem_hg.engine.graph.algebraic_connectivity()
...
def _test_fiedler_vector(t: EquivRunner) -> None:
    ...
    h3_ids, h3_fv = mem.engine.graph.fiedler_vector()
...
def _test_spectral_bisection(t: EquivRunner) -> None:
    ...
    h3_bisect = mem.engine.graph.spectral_bisection()
...
def _test_multiorder_laplacian_eigenvalues(t: EquivRunner) -> None:
    mem = build_hypergraph_h3()
    sigmas = {2: 1.0, 3: 0.5}
    eigs = mem.engine.graph.multiorder_laplacian_eigenvalues(sigmas=sigmas)
    t.check("multiorder_eigs/returns_array", isinstance(eigs, np.ndarray))
    t.check("multiorder_eigs/len_matches_nodes", len(eigs) == mem.engine.graph.node_count)
